...
class Afip {
	...
	{
		$archivo = dirname(__DIR__)."/conf.ini";
		$contenido = parse_ini_file($archivo, true);

		$this ->cert_produccion=$contenido['Afip']['produccion'];
		...
		if($this->cert_produccion == false){
			$this->CERT 		= $this->RES_FOLDER.'homologacion/'.$options['cert'];
			$this->PRIVATEKEY 	= $this->RES_FOLDER.'homologacion/'.$options['key'];
		}else{
			$this->CERT 		= $this->RES_FOLDER.'produccion/'.$options['cert'];
			$this->PRIVATEKEY 	= $this->RES_FOLDER.'produccion/'.$options['key'];
		}
